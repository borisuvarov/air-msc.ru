...
def get_data_for_chart(request):
    ...
    TRANSLIT_MAP = {
        ...
        'PM2.5 (Взвешенные частицы менее 2.5 мкм)': 'PM25',
        'PM10_GRIMM': 'PM10',
        'PM25_GRIMM': 'PM25',
    ...
    if station in clean_subscribitions_dict:
        ...
        cur.execute("SELECT DATE(checktime), substance, MAX(concentration) FROM mosecomon WHERE station='"
                    + station + "'" + "AND DATE(checktime) >= now() - INTERVAL '60 DAY' GROUP BY substance, DATE(checktime) ORDER BY DATE(checktime);")
        data = cur.fetchall()
        cur.close()
        conn.close()
        data_provider = {}
